...
export function registerJobRoutes(app: FastifyInstance) {
  ...
  /** Distinct subcategories & local areas for advanced filters (register before /jobs/:id) */
  app.get<{
    Querystring: { country?: string; mainCategory?: string };
  }>(`${P}/jobs/filter-options`, async (req) => {
    ...
    if (main && MAIN.includes(main as (typeof MAIN)[number])) {
      where.mainCategory = main;
    }
    const rows = await prisma.job.findMany({
      where,
      select: { subCategory: true, localArea: true },
    });
    const subFromJobs = [...new Set(rows.map((r) => r.subCategory))];
    const areasFromJobs = rows
      .map((r) => r.localArea)
      .filter((a): a is string => Boolean(a && String(a).trim()));

    const dbSubWhere: Record<string, unknown> = {
      active: true,
      kind: 'sub_category',
      countryCode: country,
    };
    if (main && MAIN.includes(main as (typeof MAIN)[number])) {
      dbSubWhere.mainCategory = main;
    }
    const dbSubs = await prisma.jobFilterOption.findMany({
      where: dbSubWhere as { active: boolean; kind: string; countryCode: string; mainCategory?: string },
      select: { value: true },
    });
  ...
  /** Public list + optional auth for interest / liked flags */
  app.get<{
    Querystring: {
      country?: string;
      mainCategory?: string;
    ...
    if (q.interestMatch === '1' && userId) {
      ...
      const tags = u?.interests?.map((t) => t.trim()).filter(Boolean) ?? [];
      if (tags.length) {
  ...
  app.get<{
    Querystring: { country?: string; limit?: string; mainCategory?: string };
  ...
  /** Admin: single job (draft or published) */
  ...
  }>(`${P}/admin/jobs`, { preHandler: [...adminOrSuper] }, async (req, reply) => {
    const { sub } = req.user as { sub: string };
    const admin = await prisma.user.findUnique({ where: { id: sub } });
    if (!admin || !canManageJobs(admin)) {
      return reply.code(403).send({ error: 'forbidden' });
  ...
  app.patch<{
    Params: { id: string };
    Body: Record<string, unknown>;
  }>(`${P}/admin/jobs/:id`, { preHandler: [...adminOrSuper] }, async (req, reply) => {
    const { sub } = req.user as { sub: string };
    const admin = await prisma.user.findUnique({ where: { id: sub } });
    if (!admin || !canManageJobs(admin)) {
      return reply.code(403).send({ error: 'forbidden' });
    ...
    if (b.mainCategory !== undefined) {
      ...
      if (!MAIN.includes(m as (typeof MAIN)[number])) {
        return reply.code(400).send({ error: 'invalid mainCategory' });
      }
      data.mainCategory = m;
    }
    ...
    if (b.status !== undefined) {
      data.status = b.status === 'draft' ? 'draft' : 'published';
    }
  ...
  app.delete<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      const deleted = await prisma.jobLike.deleteMany({
        where: { userId: sub, jobId: job.id },
      });
      if (deleted.count === 0) {
        return reply.code(404).send({ error: 'not liked' });
      }
